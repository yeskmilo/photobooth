 ...
 * @version     $Id: shPageRewrite.php 2246 2012-01-23 09:28:15Z silianacom-svn $
 *
 */
...
if (function_exists('shLoadPluginlanguage')) {
  ...
  function shGetCustomMetaData( $nonSef) {
    ...
    if(is_null( $_tags)) {
      ...
      try {
        $data = Sh404sefHelperDb::selectObject('#__sh404SEF_meta', '*', array('newurl' => $nonSef));
  ...
  function shGetCurrentNonSef() {
    ...
    $nonSef = Sh404sefHelperGeneral::stripTrackingVarsFromNonSef( shRouter::shPageInfo()->shCurrentPageNonSef);
  ...
  function shDoTitleTags( &$buffer) {
    ...
    if ($sefConfig->shMetaManagementActivated) {
      ...
      if (!is_null($shCustomKeywordsTag)) {
        ...
        // read Joomla! keywords if none set by us
      ...
      if (!is_null($shCustomRobotsTag)) {
        ...
        if (strpos($buffer, '<meta name="robots" content="') !== false) {
          ...
          .$shPageInfo->pageRobotsTag.'" />', $buffer);
        }
        else if (!empty($shPageInfo->pageRobotsTag)) {
      ...
      if(!empty( $tagsToInsert)) {
        $buffer = shInsertCustomTagInBuffer( $buffer, '</head>', 'before', $tagsToInsert, 'first');
      }
  ...
  function shInsertOpenGraphData( &$buffer) {
    ...
    $locale = str_replace( '-', '_', JFactory::getLanguage()->get('tag', 'en-GB'));
    ...
    $openGraphData .= "\n" . '  <meta property="og:url" content="'.JURI::base() . JString::ltrim($content, '/').'" />';
