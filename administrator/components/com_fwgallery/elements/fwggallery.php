...
class JElementFWGGallery extends JElement {
	var	$_name = 'fwggallery';
...
/*		$db->setQuery('SELECT id, name, parent FROM #__fwg_projects WHERE published=1 ORDER BY parent, ordering');
		...
		return JHTML::_('select.genericlist',  $list, ''.$control_name.'['.$name.'][]', 'class="inputbox" multiple="multiple" size="10"', 'id', 'treename', $value, $control_name.$name);//ob_get_clean();
	}
}
