 ...
 * @version     $Id: editurl.php 2237 2012-01-05 19:04:24Z silianacom-svn $
 */
...
class Sh404sefModelEditurl extends Sh404sefClassBaseeditmodel {
  ...
  private function _saveUrl( $type = sh404SEF_URLTYPE_AUTO) {
    ...
    // search DB for urls pairs with same SEF url. Must be done in a case sensitive way, hence the 'binary'
    $query = 'SELECT * FROM #__redirection WHERE binary oldurl = ' . $this->_db->Quote( $row->oldurl) . ' ORDER BY rank ASC';
    $this->_db->setQuery($query);
      ...
      if (!$sefConfig->shRecordDuplicates) {  // we don't allow duplicates : reject this URL
        ...
        if (empty( $existingRecord)) {  // this new non-sef does not already exists
          ...
          if (!empty($previousSefUrl) && strpos($this->_data['shAliasList'], $previousSefUrl) === false) {
            ...
            $query = 'SELECT count(id) FROM #__redirection WHERE binary oldurl = ' . $this->_db->Quote( $previousSefUrl);
            $this->_db->setQuery($query);
        ...
        if (!empty( $previousSefUrl) && $previousSefUrl != $row->newurl) {
          ...
          $query = 'SELECT id FROM #__redirection WHERE binary oldurl = ' . $this->_db->Quote( $previousSefUrl) . ' ORDER BY rank ASC';
          $this->_db->setQuery($query);
