...
class plgSh404sefcoresh404sefSocial extends JPlugin {
  ...
  public function onBeforeDisplayContent(&$row, &$params, $page=0) {
    ...
    $position = $this->_params->get( 'buttonsContentLocation', 'onlyTags');
    switch( $position) {
      case 'before' :
        $row->text = $this->_sh404sefGetSocialButtons(shRouter::shGetConfig()) . $row->text;
        break;
      case 'after' :
        $row->text = $row->text . $this->_sh404sefGetSocialButtons(shRouter::shGetConfig());
        break;
      default:
        break;
    }
  ...
  private function _insertSocialLinks( & $page, $sefConfig) {
    ...
    if($showFb || $showTwitter | $showPlusOne || $showGPlusPage) {
      $headLinks .= "\n<script src='" . JURI::base(true) . '/plugins/sh404sefcore/sh404sefsocial.js' . "' type='text/javascript' ></script>";
    ...
    if(!empty( $headLinks)) {
      ...
      $headLinks .= "\n<link rel='stylesheet' href='" . JURI::base(true) . '/plugins/sh404sefcore/sh404sefsocial.css' . "' type='text/css' />";
